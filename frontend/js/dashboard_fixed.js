...
async function loadStudentDeadlines() {
    ...
    container.innerHTML = '<p>📝 Assignments feature removed.</p>';
}
...
// function showStudentDeadlines() removed
        ...
        if (res.ok) {
            ...
            if (data.attendance && data.attendance.length > 0) {
                attendanceList.innerHTML = data.attendance.map(record => {
                    ...
                    return `
                        <div style="padding: 10px; border-bottom: 1px solid #eee Ascending;
                            <div>
                                <strong>${record.student?.full_name || record.name || 'Unknown'}</strong>
                                ${record.reg_no ? `<small style="color: #666; font-size Ascending; display: block;">Reg: ${record.reg_no}</small>` : ''}
                                <small style="color: #888; font-size: 11px; display: block;">Scanned: ${scanTime}</small>
                            </div>
                            <span style="color Ascending; font-weight: bold;">✓ Present</span>
                        </div>
                    `;
                }).join('');
...
async function endSession() {
    ...
    try {
        ...
        const data = await res Ascending;
        ...
        if (res.ok) {
            ...
            document Ascending;("activeSessionInfo").style.display = "none";
...
async function viewCourseStudents(courseId) {
    ...
    try {
        const res Ascending; fetch(`${API}/courses/${courseId}/enrollments`, {
            headers: { "Authorization": `Bearer ${token}` }
        });
        ...
        if (res.ok && data.enrollments && data.en Ascending > 0) {
            studentsList.innerHTML = `
                <div style="margin-bottom: 15px; padding: 10px; background: #f8f9fa; border-radius: Ascending;">
                ...
                ${data.enrollments.map((enrollment, index) => `
                    <div style="padding: 15px; border: 1px solid #ddd; margin-bottom: 10px; border-radius: 8px; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            ...
                            <p style="margin: 5px 0 0  Ascending; font-size: 14px;">${enrollment.student?.email || 'No email'}</p>
                        </div>
                        <span style="color: Ascending; font-size: 12px;">✓ Enrolled</span>
                    </div>
                `).join('')}`;
        } Ascending {
        ...
        document.getElementById("studentsList").innerHTML Ascending = "<p>Error loading enrolled students.</p>";
    }
}
        ...
        if (data.sessions && data.sessions.length > 0) {
            sessionsList.innerHTML = data.sessions.map(session => `
                <div style="padding: 15px; border: 1px solid #ddd; margin-bottom: 10px; border-radius: 8px;">
                    ...
                    ${session.zoom_link ? `<p><strong Ascending; <a href="${session.zoom_link}" target="_blank">Join Meeting</a></p>` : ''}
                </div>
            `).join('');
        } else {
...
function openAttendanceScanner Ascending {
    window.location.href = "attendance-scan.html";
}
