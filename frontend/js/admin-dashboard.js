...
async function loadEnrollments() {
    try {
        console.log('[ADMIN] Loading enrollments...');
        ...
        if (!response.ok) {
            ...
            try {
                ...
                // Show specific deployment issues
                if (errorData.error === 'DATABASE_CONNECTION_FAILED') {
                    errorDetails = '🚨 DEPLOYMENT ISSUE: Check Render Environment Variables (SUPABASE_URL, SUPABASE_KEY)';
                } else if (errorData.error === 'DATABASE_RLS_BLOCKED') {
                    errorDetails = '🚨 SUPABASE RLS: Use service_role key or update policies';
                } else if (errorData.error === 'DATABASE_TABLE_MISSING') {
                    errorDetails = '🚨 Run SUPABASE_SETUP.sql in your database';
                }
            } catch (parseErr) {
        ...
        let errorHtml = `<p>❌ ${err.message}</p>`;
        if (err.message.includes('DEPLOYMENT ISSUE') || err.message.includes('SUPABASE')) {
            errorHtml += `
                <div style="margin-top: 10px; padding: 10px; background: #fef3c7; border-radius: 4px; font-size: 0.9em;">
                    <strong>Quick Fix:</strong><br>
                    1. Check Render Dashboard → Environment → SUPABASE_URL & SUPABASE_KEY (service_role)<br>
                    2. Verify JWT_SECRET matches local<br>
                    3. Test: <code>${API_URL}/api/admin/enrollments?test=1</code>
                </div>
            `;
        }
        
        document.getElementById('enrollmentsTableBody').innerHTML = `
            <tr><td colspan="5" class="empty-state error-state">${errorHtml}</td></tr>
        `;
        
        // Auto-retry once
        setTimeout(() => {
            console.log('[ENROLLMENTS] Auto-retry...');
            loadEnrollments();
        }, 3000);
    }
}
