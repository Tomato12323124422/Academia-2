...
// GET ALL ENROLLMENTS - ENHANCED ERROR HANDLING + TEST MODE
router.get('/enrollments', (req, res) => {
  // TEST MODE: Bypass auth for debugging (remove in production)
  if (req.query.test === '1') {
    console.log('[TEST MODE] Enrollments endpoint accessed without auth');
    return handleEnrollmentsQuery(req, res, true);
  }
  return adminMiddleware(req, res, () => handleEnrollmentsQuery(req, res, false));
});
  ...
  try {
    ...
    if (err.message.includes('relation')) {
      return res.status(500).json({ 
        ...
        message: 'Enrollments table not found. Run SUPABASE_SETUP.sql',
        details: err.message 
      });
    }
